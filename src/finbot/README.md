...
- `algorithm(input_df, start, end, interval='1d')`: Implements a moving average crossover strategy
  ...
  - Returns DataFrame with signals and execution prices
- `get_buy_list_for_date()`: Returns list of stocks to buy for a specific date
- Supports customizable trading strategies and signal generation
...
- State Variables:
  ...
  - `equity`: List of portfolio snapshots:
    - Date: Valuation date
    - Equity: Total portfolio value
    - Cash: Available cash
    - PosValue: Value of all positions
- Key functions:
...
### 6. Reply Processing (`reply_listener.py`)
...
The default trading strategy implemented in `algorithm()` uses a Moving Average Crossover system:
- Calculates 20-day Simple Moving Average (SM20)
- Calculates 50-day Simple Moving Average (SM50)
- Generates Buy signal when SM20 crosses above SM50
- Generates Sell signal when SM20 crosses below SM50
- Generates Hold signal when no crossover occurs
- Execution price is set to the next day's opening price